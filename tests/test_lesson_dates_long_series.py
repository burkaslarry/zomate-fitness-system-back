...
Logic: ``get_lesson_dates_for_course`` must return ``total_lessons`` dates (cap 120), not a hard-coded 10.
"""
...
def test_enumerate_lesson_dates_40_mondays() -> None:
    start = date(2026, 5, 18)  # Monday
    dates = enumerate_lesson_dates(start, [0], 40)
    assert len(dates) == 40
    assert dates[0] == start
    assert all(d.weekday() == 0 for d in dates)


def test_get_lesson_dates_matches_total_lessons_40() -> None:
    ...
    assert len(ld) == 40
    assert ld[-1] == date(2027, 2, 15)


def test_get_lesson_dates_schema_cap_120() -> None:
    start = date(2026, 1, 5)
    course = SimpleNamespace(
        lesson_weekdays="0",
        series_start_date=start,
